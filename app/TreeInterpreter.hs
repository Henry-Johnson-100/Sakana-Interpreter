...
-- -- import SakanaParser as SyntaxUnit
-- import qualified SakanaParser
-- import qualified SakanaParser as SyntaxUnit
...
evaluatePrimaryClownfish :: Env.LampreyClosureT IO Env.Lamprey -> IO String -> IO D.Data
evaluatePrimaryClownfish mainIOClownfish envArgs = do
  mainClownfish <- Env.runLampreyClosure mainIOClownfish
  argString <- envArgs
  let argLamprey =
        bindLampreyToStringAsId "_args" ((genericDataUnit . D.String) argString)
      updatedMainClownfish =
        Env.updateClosureTable
          (Env.singletonLampreyTable argLamprey)
          (mainClownfish)
  return D.Null

bindLampreyToStringAsId :: String -> SakaPar.SyntaxUnit -> Env.LampreyBinding
bindLampreyToStringAsId lid =
  Env.LampreyBinding
    ((genericDataUnit . D.Id) lid)
    . Env.Lamprey []
    . Tree.tree

genericDataUnit :: D.Data -> SakaPar.SyntaxUnit
genericDataUnit = SakaPar.genericSyntaxUnit . SakaPar.Data
